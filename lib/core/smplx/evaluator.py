...
from lib.utils.ik_utils import batch_rodrigues
from lib.si.SI import SMPLX_SI_LR
from lib.pr.smplx.AP import SMPLX_AP_V1, SMPLX_AP_V2
from lib.ik.smplx.AnalyIK import SMPLX_AnalyIK_V1
from lib.ik.smplx.MixIK import get_body_part_func
from lib.utils.si_utils import get_bl_33_from_pos

from lib.core.smplx.config import SMPLX_SI_DATA_PATH
...
class Evaluator():
    def __init__(
        ...
        data = np.load(SMPLX_SI_DATA_PATH, allow_pickle=True)['lr'].item()
        ...
        for i, target in enumerate(self.valid_loader):
            ...
            with torch.no_grad():
                ...
                smplx_out = self.smplx(
                    betas = betas,
                    global_orient = gt_global_orient_rotmat, body_pose = gt_body_pose_rotmat,
                ...
                gt_bl = get_bl_33_from_pos(p_pos)
                ...
                t0_pos = smplx_out.joints.contiguous()
           
                #-------------------SMPLX-SI-----------------------------------
                pred_betas = SMPLX_SI_LR(t0_pos, p_pos, self.A1)
                ...
                pred_bl = get_bl_33_from_pos(t_pos)
                
                #------------------SMPLX-AP------------------------------------
                q_pos = SMPLX_AP_V1(t_pos, p_pos)
                
                #------------------SMPLX-AnalyIK-------------------------------
                analyik_global_orient_rotmat, analyik_body_pose_rotmat, analyik_lhand_pose_rotmat, analyik_rhand_pose_rotmat = SMPLX_AnalyIK_V1(t_pos, q_pos)
                ...
                ### 理论上限
                theory_body_pose_rotmat = self.leg_refine_func(t_pos, q_pos, analyik_global_orient_rotmat, analyik_body_pose_rotmat, gt_leg_twist_phi)
                theory_body_pose_rotmat = self.spine_refine_func(t_pos, q_pos, analyik_global_orient_rotmat, theory_body_pose_rotmat, gt_spine_twist_phi)
                theory_body_pose_rotmat, theory_lhand_pose_rotmat, theory_rhand_pose_rotmat = self.arm_refine_func(t_pos, q_pos, analyik_global_orient_rotmat, 
                ...
                smplx_out = self.smplx(
                    betas = pred_betas,
                    global_orient = global_orient_rotmat_0, body_pose = analyik_body_pose_rotmat,
                    left_hand_pose = analyik_lhand_pose_rotmat, right_hand_pose = analyik_rhand_pose_rotmat,
                    expression = expression_0, jaw_pose = jaw_pose_0,
                    leye_pose = leye_pose_0, reye_pose = reye_pose_0,
                    )
                p0_pos = smplx_out.joints.contiguous() 
                p0_pos = p0_pos - p0_pos[:,0:1]
                ### SMPLX_HybrIK
                pred_leg_twist_phi, pred_spine_twist_phi, pred_arm_hand_twist_phi = self.generator(p0_pos, pred_betas)
                ...
                hybrik_body_pose_rotmat = self.leg_refine_func(t_pos, q_pos, analyik_global_orient_rotmat, analyik_body_pose_rotmat, pred_leg_twist_phi)
                hybrik_body_pose_rotmat = self.spine_refine_func(t_pos, q_pos, analyik_global_orient_rotmat, hybrik_body_pose_rotmat, pred_spine_twist_phi)
                hybrik_body_pose_rotmat, hybrik_lhand_pose_rotmat, hybrik_rhand_pose_rotmat = self.arm_refine_func(t_pos, q_pos, analyik_global_orient_rotmat, 
